...
from tkinter import font, messagebox, Menu, colorchooser, Frame, simpledialog
...
class Ratio_Calculator:
    def __init__(self):
        ...
        def validate_Entries(string):# Allow Only Numeric values, negative sign and decimal point for Entry Widgets
            ...
            # Prevent duplicates
            or (string.count('-') <= 1
                    and string.count('.') <= 1 # allow only one decimal point
        ...
        def calculate_ratio(a_1,a_2,b_1,b_2,a_3,b_3):
            try:
                ...
                if count == 4: # All ratio value present, examine for porportion values
                    ...
                    Kfactor_ba=round(abs(bsize / asize),3) #X Amount Of B's Per X Amount Of A's
                    KDisplay_ba="K = Abs(B range / A range)"
                    Kfactor_ab=round(abs(asize / bsize),3) #X Amount Of A's Per X Amount Of B's
        ...
        width=screen_width/5 # width of the main
        height=root_hgt
        x=root.winfo_x()
        y=root.winfo_y()
    ...
    if which=='dp':
        ...
        if bbs is not None:
            if str(bbs) in choices:
               ...
               showinfo('Binary Bit Size', f'The Value You Entered "{bbs}" Is Incorrect. Please Try Again!')
...
def choose_color(which): # Display fg and bg Colors
    if which=='bg':# Background
        ...
        Display_Text.config(fg=Display_fg.get())
def about():
    showinfo('About pySci Calculator', 'Creator: Ross Waters\nEmail: RossWatersjr@gmail.com'\
        ...
        '\nRevision \ Date: 2.2.9 \ 12/17/2022'\
...
if __name__ == "__main__":
    ...
    hyp_btn=tk.Button(pad_fra, text='Hyp', background="green", foreground = 'whitesmoke', # Trig Hyp Button
        borderwidth=5, relief="raised", font=root.font)
    hyp_btn.place(relx=x1, rely=y, relwidth=wid, relheight=hgt)
    ...
    arc_btn=tk.Button(pad_fra, text='Arc', background="green", foreground = 'whitesmoke', # Trig Arc Button
        borderwidth=5, relief="raised", font=root.font)
    arc_btn.place(relx=x2, rely=y, relwidth=wid, relheight=hgt)
